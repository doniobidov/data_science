...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeActionFromQValues(self, state):
        ...
        for a in self.getLegalActions(state):
            if max_q < self.getQValue(state, a):
                max_q = self.getQValue(state, a)
                best_actions = [a]
            elif max_q == self.getQValue(state, a):
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def getQValue(self, state, action):
        ...
        for feature in self.featExtractor.getFeatures(state, action):
            q = q + self.weights[feature] * self.featExtractor.getFeatures(state, action)[feature]
        return q
        util.raiseNotDefined()
    ...
    def update(self, state, action, nextState, reward):
        ...
        for feature in self.featExtractor.getFeatures(state, action):
          self.weights[feature] = self.weights[feature] + self.alpha * delta * self.featExtractor.getFeatures(state, action)[feature]
