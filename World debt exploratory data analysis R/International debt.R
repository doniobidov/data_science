...
df <- read.csv('C:\\Users\\Doniyor\\Desktop\\Analyze International Debt Statistics\\datasets\\international_debt.csv')
attach(df)
# summary statistics
summary(df)
sd(debt)
...
sum(debt)
# total amount of debt owed by each country
country_debts <- aggregate(debt, list(country_name), FUN=sum)
write.table(country_debts, file = "country_debts.txt", sep = "\t", row.names = TRUE, col.names = NA)
